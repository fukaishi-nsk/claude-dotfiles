...
4. 文面を組み立てる（現行手動運用と同じ・カレンダー招待テキストの型。絵文字なし・挨拶文なし）:
   ```
   [★nanco]そら植物園
   8月 24日 (月曜日) · 15:00～16:00
   タイムゾーン: Asia/Tokyo
   Google Meet の参加に必要な情報
   ビデオ通話のリンク: https://meet.google.com/xxx-xxxx-xxx
   ```
   日付・曜日・時刻・リンクはイベントから取る。ダイヤルイン行はMCPから取得できないため省略してよい
   （手動運用時代の文面には含まれていたが、リンクが本体）。
5. 送信:
...
## スケジュール（登録は claude.ai routines・bridge環境「カスタマー」）
