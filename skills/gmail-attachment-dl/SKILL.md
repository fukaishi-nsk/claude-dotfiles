...
- 代わりに**専用の永続プロファイル**を全コマンドに毎回付ける: `--profile "$HOME/.agent-browser/profiles/gmail"`（⚠ユーザー名がPCごとに違うため$HOMEで書くこと。同一パスで使い続ける限り暗号化は自己整合するので、closeしてもログインは残る。ログイン済み: nsketch機2026-07-30・sinse機2026-08-14・Mac 2026-09-07。**2026-09-07からMacも同じ方式に統一**）
...
2. **URL直開き**:
   ```
   agent-browser --profile "$HOME/.agent-browser/profiles/gmail" open "https://mail.google.com/mail/u/0/#all/<messageIdHex>"
   ```
   出力タイトルが「nsketch.com メール」（正しいアカウント）であることを確認
3. **添付ボタンのrefを特定**:
   ```
   agent-browser --profile "$HOME/.agent-browser/profiles/gmail" snapshot -i -c | grep ダウンロード
   ```
   「添付ファイル ◯◯ をダウンロード」ボタンの `@eN` を拾う
4. **保存先を指定してダウンロード**:
   ```
   agent-browser --profile "$HOME/.agent-browser/profiles/gmail" download @eN "<保存先の絶対パス/ファイル名>"
   ```
...
7. **後始末**: `agent-browser --profile "$HOME/.agent-browser/profiles/gmail" close`（ログイン状態はプロファイルに残る）
