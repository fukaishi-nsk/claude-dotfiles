...
2. さらにGAS版（毎時・Drive API改名）が新着へ `YYMMDD_`（着弾日ベース＝Drive作成日のJST）を自動前置する（正本: dotfiles `gas/notta-inbox-datestamp/`。2026-08-28〜。旧Mac mini launchd版は機能していない→上の「日付プレフィックス自動付与」）。付与まで最大1時間かかる（2026-09-24確認: 8/28以降の着弾は全件付与済み）。⚠着弾日ベースのため長時間録音の翌日処理では会議日と1日ズレることがある→仕分け時に会議文脈で補正する
